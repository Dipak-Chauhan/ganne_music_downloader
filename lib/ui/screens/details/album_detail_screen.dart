...
class _AlbumDetailScreenState extends ConsumerState<AlbumDetailScreen> {
  ...
  void _fetchAlbumDetails() async {
    try {
      final data = await ref.read(qobuzServiceProvider).getAlbumInfo(widget.album.id!);
      if (mounted) setState(() { _fetchedData = data; _isLoading = false; });
    } catch (e) {
  ...
  void _queueTrack(QobuzTrack track) {
    ...
    service.queueTrack(
      ...
      year: widget.album.releasedAt != null 
          ? DateTime.fromMillisecondsSinceEpoch(widget.album.releasedAt! * 1000).year 
          : null,
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(widget.album.title, style: tt.bodyMedium?.copyWith(color: cs.onSurfaceVariant), textAlign: TextAlign.center),
              const SizedBox(height: 20),
              FilledButton.tonalIcon(
                onPressed: () { Navigator.pop(ctx); _downloadAlbumIndividual(); },
              ...
              OutlinedButton.icon(
                onPressed: () { Navigator.pop(ctx); _downloadAlbumZip(); },
  ...
  void _downloadAlbumIndividual() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        action: SnackBarAction(label: 'View Queue', onPressed: () => Navigator.pop(context)),
      ),
    );
  }
      ...
      body: _isLoading
          ...
          : _error != null
              ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Padding(padding: const EdgeInsets.symmetric(horizontal: 32), child: Text(_error!, style: tt.bodyMedium?.copyWith(color: cs.error), textAlign: TextAlign.center)),
                ]))
              : CustomScrollView(
                  slivers: [
                    ...
                    SliverAppBar.large(
                      ...
                      flexibleSpace: FlexibleSpaceBar(
                        title: Text(widget.album.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: tt.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
                        background: Stack(fit: StackFit.expand, children: [
                          CachedNetworkImage(
                            imageUrl: widget.album.getFullResImageUrl(),
                            fit: BoxFit.cover,
                            placeholder: (_, _a) => Container(color: cs.surfaceContainerHighest),
                            errorWidget: (_, _a, _b) => Container(color: cs.surfaceContainerHighest, child: Center(child: Icon(Icons.album, size: 64, color: cs.outlineVariant))),
                          ),
                          DecoratedBox(
                            decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter,
                              colors: [Colors.transparent, cs.surface.withAlpha(200), cs.surface], stops: const [0.3, 0.75, 1.0])),
                          ),
                        ]),
                      ),
                    ),
                    ...
                    SliverToBoxAdapter(
                      child: Padding(
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(widget.album.artist?.name ?? 'Unknown Artist', style: tt.titleMedium?.copyWith(color: cs.primary, fontWeight: FontWeight.w600)),
                          const SizedBox(height: 12),
                          Wrap(spacing: 8, runSpacing: 6, children: [
                            if (widget.album.tracksCount != null)
                              Chip(
                                avatar: Icon(Icons.queue_music, size: 16, color: cs.onSurfaceVariant),
                                label: Text('${widget.album.tracksCount} tracks'),
                            ...
                            if (widget.album.duration != null)
                              Chip(
                                ...
                                label: Text(_formatDuration(widget.album.duration)),
                                materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
                                visualDensity: VisualDensity.compact,
                              ),
                            if (widget.album.hires == true)
                              Chip(
                                avatar: Icon(Icons.high_quality, size: 16, color: cs.onTertiaryContainer),
                                label: Text('Hi-Res', style: TextStyle(color: cs.onTertiaryContainer)),
                            ...
                            if (widget.album.maximumBitDepth != null)
                              Chip(
                                avatar: Icon(Icons.memory, size: 16, color: cs.onSurfaceVariant),
                                label: Text('${widget.album.maximumBitDepth}-Bit / ${widget.album.maximumSamplingRate} kHz'),
                                materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
                                visualDensity: VisualDensity.compact,
                              ),
                          ]),
                        ...
                        child: Row(children: [
                          Expanded(
                            child: DropdownMenu<String>(
                              ...
                              onSelected: (val) { if (val != null) setState(() => _selectedQuality = val); },
                              dropdownMenuEntries: _qualities.map((q) => DropdownMenuEntry(value: q['id']!, label: '${q['label']} ${q['detail']}')).toList(),
                            ),
                          ),
                          const SizedBox(width: 12),
                          FilledButton.icon(onPressed: _showDownloadOptions, icon: const Icon(Icons.download_rounded), label: const Text('Download')),
                        ]),
                      ),
                    ),
                                ...
                                children: [
                                  InkWell(
                                    ...
                                    child: Padding(
                                      ...
                                      child: Row(children: [
                                        SizedBox(width: 28, child: Text('${track.trackNumber ?? index + 1}', style: tt.titleSmall?.copyWith(color: cs.onSurfaceVariant, fontWeight: FontWeight.w600), textAlign: TextAlign.center)),
                                        const SizedBox(width: 16),
                                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                          Text(track.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: tt.titleSmall?.copyWith(fontWeight: FontWeight.w500)),
                                          if (track.performer != null && track.performer?.name != widget.album.artist?.name)
                                            Text(track.performer!.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: tt.bodySmall?.copyWith(color: cs.onSurfaceVariant)),
                                        ])),
                                        Text(_formatDuration(track.duration), style: tt.bodySmall?.copyWith(color: cs.onSurfaceVariant)),
                                        ...
                                        if (isDownloading)
                                          SizedBox(width: 40, height: 40, child: Stack(alignment: Alignment.center, children: [
                                            ...
                                            Text('${(progress * 100).toInt()}', style: tt.labelSmall?.copyWith(fontSize: 8, fontWeight: FontWeight.w700, color: cs.primary)),
                                          ]))
                                        else if (isCompleted)
                                          ...
                                          Icon(Icons.download_outlined, color: cs.primary, size: 22),
                                      ]),
                                    ),
                                  ),
                                  if (isDownloading)
